    ...
    async def delete(self, button: discord.ui.Button, interaction: discord.Interaction):
        if self.user_id and interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "You can't delete this message!", ephemeral=True
            )
            return
...
class Owner(commands.Cog):
    ...
    async def get_season_file(self, ctx: commands.Context):
        ...
        database_file = discord.File(
            io.BytesIO(json.dumps(asdict(season), indent=4).encode("utf-8"))
        )
        database_file.filename = "winter_2025.json"
        expire_datetime = datetime.datetime.now(datetime.UTC) + datetime.timedelta(
            seconds=30
        )
        await ctx.reply(
    ...
    async def clear_cache(self, ctx: commands.Context):
        ...
        self.logger.info(
            f"Cache has been purged by @{ctx.author.name} ({ctx.author.id})"
        )
        await ctx.reply(
    ...
    @commands.is_owner()
    async def get_users(
        self, ctx: commands.Context, rep: str = "*", passed_status: str = "*"
    ):
        season, _ = await get_season_data()
        ...
        if passed_status not in ["all", "*", "passed", "failed", "pending"]:
            await ctx.reply(
                "Invalid status! Use `all`, `*`, `passed`, `failed` or `pending`"
            )
            return
        if rep not in ["all", "*"] and rep not in [
            user.rep.replace(" ", "-").lower() for user in season.users.values()
        ]:
        ...
        for username, user in season.users.items():
            if (
                passed_status not in ["all", "*"]
                and user.status.lower() != passed_status
            ):
                continue
        ...
        usernames_file = discord.File(
            io.BytesIO("\n".join(pending_usernames).encode("utf-8"))
        )
        usernames_file.filename = "usernames.txt"

        if len(pending_ids) != 0:
            mentions_file = discord.File(
                io.BytesIO(
                    "\n".join([f"<@{userid}>" for userid in pending_ids]).encode(
                        "utf-8"
                    )
                )
            )
        ...
        await ctx.reply(
            ...
            files=[usernames_file, mentions_file]
            if len(pending_ids) != 0
            else [usernames_file],
        )
    ...
    async def delete_message(
        self, ctx: commands.Context, message_id: int, channel_id: int = None
    ):
        message = self.bot.get_message(message_id)
        if not message:
            if channel_id is None:
                await ctx.reply(
                    "Channel ID argument required for uncached message!", delete_after=3
                )
                return

...
class User:
    ...
    def get_contractee(self, season: "Season") -> Optional["User"]:
        users_with_this_contractor = [
            user for user in season.users.values() if user.contractor == self.name
        ]
        return (
            users_with_this_contractor[0]
            if len(users_with_this_contractor) > 0
            else None
        )
...
def _get_season_dashboard_data(sheet_data) -> Season:
    ...
    for row in rows:
        ...
        for i in range(len(contract_names)):
            ...
            passed_contract = (
                True
                if len(contract_passed) > i and contract_passed[i] == "PASSED"
                else False
            )
            if passed_contract:
...
def _add_basechallenge_data(season: Season, sheet_data):
    ...
    for row in rows:
        ...
        if "Challenge Contract" in user.contracts:
            ...
            challenge_contract.progress = (
                row[30] if len(row) > 30 and row[30] != "" else "?/?"
            )
            challenge_contract.rating = row[28]
            challenge_contract.review_url = _get_first_url(
...
def _add_specials_data(season: Season, sheet_data):
    ...
    for row in rows:
        ...
        extreme_special.progress = (
            "Completed" if row[0] == "PASSED" else "Not Completed"
        )
    ...
    for row in rows:
        ...
        if "Challenge Buddy" in user.contracts:
            ...
            buddy_challenge.review_url = (
                _get_first_url(row[13]) if len(row) > 13 else ""
            )
...
async def get_season_sheet_session() -> aiohttp.ClientSession:
    ...
    if _season_sheet_session is None or _season_sheet_session.closed:
        _season_sheet_session = aiohttp.ClientSession(
...
async def get_season_data() -> tuple[Season, float]:
    ...
    async with session.get(
        ...
        return _convert_sheet_to_season(sheet_data), datetime.datetime.now(
            datetime.UTC
        ).timestamp()
